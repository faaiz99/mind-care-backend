...
export const createPaymentIntent: RequestHandler = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    res.status(200).json({status:'success', message: 'Payment Intent Created', url: data })
  } catch (error) {
    next(error)
  }
}

export const getPaymentsClient:RequestHandler = async (req:Request, res:Response, next:NextFunction)=>{
  try {
