...
export const getNotificationsTherapist = async (id: string) => {
	...
	if (!response)
		throw new Error(`Therapist notifications not found: ${id}`)
	return response
}
...
export const getNotificationsClient = async (id: string) => {
	...
	if (!response)
		throw new Error(`Client notifications not found: ` + id)
	return response
}
...
export const deleteNotificationById = async (id: string) => {
	...
	if (!response)
		throw new Error(`Notification Deleted: ${id}`)
	return response
}

// eslint-disable-next-line @typescript-eslint/no-explicit-any
export const createNotificationClient = async (notification: any, id: string) => {
	const response = await Notification.create({
		clientId: id,
		...notification
	})
	if (!response)
		throw new Error(`Notification Created: ${id}`)
	return response
}

// eslint-disable-next-line @typescript-eslint/no-explicit-any
export const createNotificationTherapist = async (notification: any, id: string) => {
	const response = await Notification.create({
		therapistId: id,
		...notification
	})
